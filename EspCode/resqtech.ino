 ...
 * - In Arduino IDE, go to Sketch > Include Library > Manage Libraries... and search for "Firebas/*********************************************************************************************************************
 * Description: This sketch reads data from DHT11 (Temperature & Humidity), a flame sensor, and an MQ-2 gas sensor.
 * It then sends these values to a Firebase Realtime Database using an ESP8266 (NodeMCU).
 *
 * THIS VERSION USES THE Firebase_ESP_Client.h and dht11.h LIBRARIES.
...
void loop() {
  ...
  // Create a FirebaseJson object to structure the data.
  FirebaseJson json;
  json.set("temperature", temperature);
  json.set("humidity", humidity);
  json.set("flame_detected", flame_detected);
  json.set("gas_level", gas_value);

  // The path in your Firebase database where the data will be stored.
  String path = "/sensor_data";

  Serial.printf("Sending data to Firebase path: %s\n", path.c_str());

  // Use setJSON to send the entire JSON object to the specified path.
  // This will overwrite any data already at that path.
  // We pass the Firebase Data Object (fbdo) by reference.
  if (Firebase.RTDB.setJSON(&fbdo, path, &json)) {
    Serial.println("SUCCESS: Data sent to Firebase.");
    // You can see the data that was sent by printing fbdo.payload()
    // Serial.println("PAYLOAD: " + fbdo.payload());
  } else {
    Serial.println("ERROR: Failed to send data to Firebase.");
